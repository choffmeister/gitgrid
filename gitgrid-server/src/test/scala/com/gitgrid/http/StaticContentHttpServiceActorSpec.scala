...
class StaticContentHttpServiceActorSpec extends Specification with AsyncUtils with RequestUtils {
  "StaticContentHttpServiceActor" should {
    "serve index.html" in new TestActorSystem with HttpTestEnvironment {
      ZipUtils.unzip(getClass.getResourceAsStream("/web.zip"), httpConf.webDir.get)
      implicit val httpService = TestActorRef(new HttpServiceActor(coreConf, httpConf, db))
    ...
    "server index.html for GET requests to non-existent files" in new TestActorSystem with HttpTestEnvironment {
      ZipUtils.unzip(getClass.getResourceAsStream("/web.zip"), httpConf.webDir.get)
      implicit val httpService = TestActorRef(new HttpServiceActor(coreConf, httpConf, db))
    ...
    "serve static content" in new TestActorSystem with HttpTestEnvironment {
      ZipUtils.unzip(getClass.getResourceAsStream("/web.zip"), httpConf.webDir.get)
      implicit val httpService = TestActorRef(new HttpServiceActor(coreConf, httpConf, db))
